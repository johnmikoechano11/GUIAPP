...
import main_app.TransactionForm;
 ...
 * @author Angie
 */
public class Transaction_page extends javax.swing.JInternalFrame {
    ...
    transaction_table.setShowGrid(false);
    transaction_table.setIntercellSpacing(new java.awt.Dimension(0,0));
    transaction_table.setBackground(Color.WHITE);
    transaction_table.setBorder(null);


    transaction_table.getTableHeader().setOpaque(false);
    transaction_table.getTableHeader().setPreferredSize(new java.awt.Dimension(100, 35));

transaction_table.getTableHeader().setDefaultRenderer(new DefaultTableCellRenderer() {
    @Override
...
transaction_table.setDefaultRenderer(Object.class, new DefaultTableCellRenderer() {
    @Override
    public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
...
public void printReceipt(String id, String member, String amount, String date, String status) {
    ...
    if (job.printDialog()) {
        try {
            ...
            javax.swing.JOptionPane.showMessageDialog(null, "Printer Error: " + ex.getMessage());
        }
    }
}
    ...
    private void initComponents() {
        ...
        jLabel5.setText("WORKOUT PLAN");
        jLabel5.setHorizontalTextPosition(javax.swing.SwingConstants.LEFT);
        header.add(jLabel5, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 0, 290, 60));
...
config.configclass conf = new config.configclass();
    ...
    String query = "SELECT "
                 ...
                 + "FROM transactions t "
                 + "LEFT JOIN members m ON t.m_id = m.m_id "
                 + "WHERE UPPER(m.m_fname) LIKE UPPER('%" + txt + "%') " 
                 + "OR UPPER(t.t_status) LIKE UPPER('%" + txt + "%') "   
                 + "OR UPPER(t.t_method) LIKE UPPER('%" + txt + "%') "  
                 + "OR CAST(t.t_id AS CHAR) LIKE '%" + txt + "%'";    

    conf.displayData(query, transaction_table);
    ...
    if(rowIndex < 0){
        ...
        stf.t_id.setText(model.getValueAt(rowIndex, 0).toString());


        stf.m_id.setSelectedItem(model.getValueAt(rowIndex, 1).toString());
        stf.p_id.setSelectedItem(model.getValueAt(rowIndex, 2).toString());

        stf.m_amount.setText(model.getValueAt(rowIndex, 3).toString());
        

        stf.method.setSelectedItem(model.getValueAt(rowIndex, 4).toString());
        
   
        stf.t_date.setText(model.getValueAt(rowIndex, 5).toString());
    ...
    private void deleteMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_deleteMouseClicked
        ...
        if(rowIndex < 0){
            ...
            if(a == JOptionPane.YES_OPTION){
                ...
                dbc.deleteData(u_id, "transactions", "t_id");
                displayData();
            }
        }
